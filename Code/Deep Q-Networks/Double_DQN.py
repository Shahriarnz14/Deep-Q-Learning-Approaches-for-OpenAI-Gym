...
class Replay_Memory():
	...
	def __init__(self, memory_size=50000, burn_in=10000):
		# The memory stores transitions recorded from the agent
	...
	def sample_batch(self, batch_size=32):
		...
		return np.asarray(sampled_transitions_batch)
...
class DQN_Agent():
	...
	# In this class, functions to do the following.
	...
	def train(self, num_episodes=1000):
		...
		# while also updating network parameters.
					...
					for data in sampled_transitions_batch:
						...
						if not data[self.IS_DONE_IDX]:
							q_next[data[self.ACTION_IDX]] += \
								...
								* self.QNN_target.model.predict(np.array([data[self.NEXT_STATE_IDX]]), batch_size=1) [0]\
									[np.argmax(self.QNN.model.predict(np.array([data[self.NEXT_STATE_IDX]]), batch_size=1)[0])]
